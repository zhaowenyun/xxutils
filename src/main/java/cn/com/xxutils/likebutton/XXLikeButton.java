package cn.com.xxutils.likebutton;
...
public class XXLikeButton extends FrameLayout implements View.OnClickListener {
    private static final DecelerateInterpolator DECCELERATE_INTERPOLATOR = new DecelerateInterpolator();
    private static final AccelerateDecelerateInterpolator ACCELERATE_DECELERATE_INTERPOLATOR = new AccelerateDecelerateInterpolator();
    private static final OvershootInterpolator OVERSHOOT_INTERPOLATOR = new OvershootInterpolator(4);

    private ImageView icon;
    private cn.com.xxutils.likebutton.XXDotsView XXDotsView;
    private cn.com.xxutils.likebutton.XXCircleView XXCircleView;
    ...
    public XXLikeButton(Context context) {
        this(context, null);
    }

    public XXLikeButton(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public XXLikeButton(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        XXDotsView = (cn.com.xxutils.likebutton.XXDotsView) findViewById(R.id.dots);
        XXCircleView = (cn.com.xxutils.likebutton.XXCircleView) findViewById(R.id.circle);

        final TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.XXLikeButton, defStyle, 0);

        iconSize = array.getDimensionPixelSize(R.styleable.XXLikeButton_icon_size, -1);
        if (iconSize == -1)
            iconSize = 40;

        String iconType = array.getString(R.styleable.XXLikeButton_icon_type);

        likeDrawable = array.getDrawable(R.styleable.XXLikeButton_like_drawable);

        if(likeDrawable!=null)
            setLikeDrawable(likeDrawable);

        unLikeDrawable = array.getDrawable(R.styleable.XXLikeButton_unlike_drawable);
        ...
        circleStartColor = array.getColor(R.styleable.XXLikeButton_circle_start_color, 0);

        if (circleStartColor != 0)
            XXCircleView.setStartColor(circleStartColor);

        circleEndColor = array.getColor(R.styleable.XXLikeButton_circle_end_color, 0);

        if (circleEndColor != 0)
            XXCircleView.setEndColor(circleEndColor);

        dotPrimaryColor = array.getColor(R.styleable.XXLikeButton_dots_primary_color, 0);
        dotSecondaryColor = array.getColor(R.styleable.XXLikeButton_dots_secondary_color, 0);

        if (dotPrimaryColor != 0 && dotSecondaryColor != 0) {
            XXDotsView.setColors(dotPrimaryColor, dotSecondaryColor);
        }
        ...
        Boolean status = array.getBoolean(R.styleable.XXLikeButton_liked,false);
        setLiked(status);
        setEffectsViewSize();
    ...
    public void onClick(View v) {
        ...
        if (isChecked) {
            ...
            icon.setScaleX(0);
            icon.setScaleY(0);
            XXCircleView.setInnerCircleRadiusProgress(0);
            XXCircleView.setOuterCircleRadiusProgress(0);
            XXDotsView.setCurrentProgress(0);

            animatorSet = new AnimatorSet();

            ObjectAnimator outerCircleAnimator = ObjectAnimator.ofFloat(XXCircleView, XXCircleView.OUTER_CIRCLE_RADIUS_PROGRESS, 0.1f, 1f);
            outerCircleAnimator.setDuration(250);
            outerCircleAnimator.setInterpolator(DECCELERATE_INTERPOLATOR);

            ObjectAnimator innerCircleAnimator = ObjectAnimator.ofFloat(XXCircleView, XXCircleView.INNER_CIRCLE_RADIUS_PROGRESS, 0.1f, 1f);
            ...
            ObjectAnimator dotsAnimator = ObjectAnimator.ofFloat(XXDotsView, XXDotsView.DOTS_PROGRESS, 0, 1f);
    ...
    public void setLikeDrawableRes(@DrawableRes int resId) {
        ...
        if (iconSize != 0) {
            likeDrawable = XXUtils_LikeButton.resizeDrawable(getContext(), likeDrawable, iconSize, iconSize);
        }
    }
    ...
    public void setLikeDrawable(Drawable likeDrawable) {
        ...
        if (iconSize != 0) {
            this.likeDrawable = XXUtils_LikeButton.resizeDrawable(getContext(), likeDrawable, iconSize, iconSize);
        }
    ...
    public void setUnlikeDrawableRes(@DrawableRes int resId) {
        ...
        if (iconSize != 0) {
            unLikeDrawable = XXUtils_LikeButton.resizeDrawable(getContext(), unLikeDrawable, iconSize, iconSize);
        }
    ...
    public void setUnlikeDrawable(Drawable unLikeDrawable) {
        ...
        if (iconSize != 0) {
            this.unLikeDrawable = XXUtils_LikeButton.resizeDrawable(getContext(), unLikeDrawable, iconSize, iconSize);
        }
        icon.setImageDrawable(unLikeDrawable);
    ...
    {
        setIconSizePx((int) XXUtils_LikeButton.dipToPixels(getContext(), (float) iconSize));
    }
    /**
     * Sets the size of the drawable/icon that's being used. The views that generate
     * the like effect are also updated to reflect the size of the icon.
    ...
    private Icon parseIconType(String iconType) {
        List<Icon> icons = XXUtils_LikeButton.getIcons();
    ...
    public void setExplodingDotColorsRes(@ColorRes int primaryColor, @ColorRes int secondaryColor) {
        XXDotsView.setColors(ContextCompat.getColor(getContext(), primaryColor), ContextCompat.getColor(getContext(), secondaryColor));
    }

    public void setCircleStartColorRes(@ColorRes int circleStartColor) {
        this.circleStartColor = circleStartColor;
        XXCircleView.setStartColor(ContextCompat.getColor(getContext(), circleStartColor));
    }

    public void setCircleEndColorRes(@ColorRes int circleEndColor) {
        this.circleEndColor = circleEndColor;
        XXCircleView.setEndColor(ContextCompat.getColor(getContext(), circleEndColor));
    }
    ...
    public void setEffectsViewSize() {
        if (iconSize != 0) {
            XXDotsView.setSize(iconSize * 3, iconSize * 3);
            XXCircleView.setSize(iconSize, iconSize);
        }
    }
