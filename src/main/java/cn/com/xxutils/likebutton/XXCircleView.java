...
public class XXCircleView extends View {
    ...
    public XXCircleView(Context context) {
        super(context);
        init();
    }

    public XXCircleView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public XXCircleView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void updateCircleColor() {
        float colorProgress = (float) XXUtils_LikeButton.clamp(outerCircleRadiusProgress, 0.5, 1);
        colorProgress = (float) XXUtils_LikeButton.mapValueFromRangeToRange(colorProgress, 0.5f, 1f, 0f, 1f);
    ...
    public static final Property<XXCircleView, Float> INNER_CIRCLE_RADIUS_PROGRESS =
            new Property<XXCircleView, Float>(Float.class, "innerCircleRadiusProgress") {
                @Override
                public Float get(XXCircleView object) {
                    return object.getInnerCircleRadiusProgress();
                }

                @Override
                public void set(XXCircleView object, Float value) {
                    object.setInnerCircleRadiusProgress(value);
                }
            };

    public static final Property<XXCircleView, Float> OUTER_CIRCLE_RADIUS_PROGRESS =
            new Property<XXCircleView, Float>(Float.class, "outerCircleRadiusProgress") {
                @Override
                public Float get(XXCircleView object) {
